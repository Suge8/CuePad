...
| `electron/` | Electron 主进程与 preload；`db.ts` 负责 `node:sqlite`、旧 Tauri 数据继承、迁移和真事务，`ipc-sql.ts` 暴露 SQL IPC，`sidecar.ts` 管理投送子进程、协议、串行队列与生命周期 |
...
| `e2e/` | 无头视觉验收：Playwright WebKit + 桌面桥 mock，SQL 走 `window.cuepad.sql`，覆盖工作区、悬浮任务、动效与响应式布局（`bun run test:e2e`） |
| `e2e-electron/` | 真实 Electron 验收：安全桥、SQLite CRUD/事务、重启恢复与旧 Tauri WAL 数据继承（`bun run test:electron`） |
