...
bun run build:sidecar # 构建 macOS 一键投送 sidecar
bun run test:sidecar  # 构建并验证 sidecar JSON 行协议
bun run test:electron # 真实 Electron + SQLite / sidecar 验收
...
- **一键投送（macOS-only）**（`native/dispatch/` / `electron/sidecar.ts` / `src/lib/workspace/dispatch.ts`）：长驻 Rust sidecar 订阅 `NSWorkspaceDidActivateApplicationNotification`，从通知 `userInfo[NSWorkspaceApplicationKey]` 提取被激活应用（事实源是通知携带的对象；回调时刻的全局 frontmost 在快速切换下会丢事件），持续记录最近一个非 CuePad 前台应用的 PID、bundle id 与本地化名，不轮询。Electron 与 sidecar 使用 stdio JSON 行协议；宿主 PID 和 bundle id 通过启动参数排除，避免 sidecar 把 Electron 自身记作目标。默认投送到上一个应用；运行中应用列表可按 bundle id 固定目标，偏好存 localStorage `cuepad:dispatch-target`，目标重启后重新解析 PID，未运行时明确报错且不回退。主进程把“权限/目标/CGEvent 预检 → 写剪贴板 → 隐藏窗口 → 激活目标并向目标 PID 发送 `Cmd+V`”作为一条串行事务；sidecar 等目标激活通知后才投递按键，并在目标消费剪贴板的无回执边界保留 100ms 有界稳定窗口，避免连续投送把前一次剪贴板覆盖成后一次；激活 2s 无通知则返回目标不可用。后半段失败会恢复 CuePad 窗口。未授权仍返回 `ACCESSIBILITY_PERMISSION_REQUIRED`，renderer 的系统设置引导文案不变。sidecar 退出后当前请求明确失败，下次调用自动重启；Electron 退出时关闭 stdin 并等待子进程退出。
...
- 重新生成（macOS 本机，无外部依赖）：`design/tools/cutout.swift` 用 Vision 抠图；`design/tools/icon.swift` 从黑白 logo 提取 mark，合成 1024 squircle（喂 `bun tauri icon` 生成全套）与 `src-tauri/icons/tray.png`（44px 黑+alpha 模板图，`lib.rs` 中 `icon_as_template(true)`）。Electron 从该图生成产品定稿的 18px `trayTemplate.png` 与 36px `trayTemplate@2x.png`；缺少密度后缀会被当成 44pt，导致菜单栏图标放大。
...
- `bun run test:electron`：先构建并冒烟验证真实 sidecar，再运行 Playwright Electron（`e2e-electron/`）；使用隔离 userData 验证安全 preload、全局快捷键注册与重启恢复、系统剪贴板、投送全链路串行/失败回显/sidecar 重启与退出回收、close=hide、拖拽区、空库迁移、真实 CRUD/事务，以及旧 Tauri WAL 数据继承与源文件不变。托盘模板图、菜单回调和 sidecar 客户端队列另由 `bun test` 覆盖。
